...
def image_provider():
    ...
    if p == "replicate":
        return ReplicateImageProvider(
            settings.replicate_api_token, settings.replicate_image_model
        )
    raise ProviderError(
        f"Unsupported image provider: {p}. Supported: mock, openai, replicate"
    )
...
def video_provider():
    ...
    if p == "replicate":
        return ReplicateVideoProvider(
            settings.replicate_api_token, settings.replicate_video_model
        )
...
def provider_status():
    def ok(name, key):
        return {"provider": name, "configured": bool(key)}

    return {
        "image": settings.image_provider,
        "video": settings.video_provider,
        "openai": ok("openai", settings.openai_api_key),
        "replicate": ok("replicate", settings.replicate_api_token),
    }
