...
class OpenAIImageProvider(ImageProvider):
    def __init__(self, key, model="gpt-image-1"):
        self.key = key
        self.model = model
    ...
    def generate(self, prompt, output, width=1024, height=1024):
        ...
        size = (
            f"{width}x{height}"
            if width in (1024, 1536) and height in (1024, 1536)
            else "1024x1024"
        )
        d = request_json(
    ...
    def edit(self, source, prompt, output):
        raise ProviderError(
            "OpenAI edit requires multipart transport; use generate or configure another editor provider"
        )
