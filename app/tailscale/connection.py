...
def status():
    ...
    try:
        return {"available": True, "status": json.loads(r.stdout)}
    except json.JSONDecodeError:
        return {"available": True, "raw": r.stdout, "error": r.stderr}


def connection_url(host):
    return f"https://{host}/{settings.auth_token}/mcp"
