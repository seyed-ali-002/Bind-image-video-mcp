...
from app.assets import stats,list_assets,get_asset,asset_manifest,export_assets
...
@mcp.tool()
def bina_capabilities():return {"name":"Bina","version":"0.7.0","phase":"E","features":["asset_delivery","secure_asset_resolution","asset_preview","asset_download","asset_export","asset_search","media_pipeline","persistent_jobs"]}
@mcp.tool()
...
@mcp.tool()
def bina_list_assets(kind=None,limit=100,query=None):return list_assets(kind,limit,query)
@mcp.tool()
def bina_get_asset(asset_id):return asset_manifest(asset_id) or {"error":"asset_not_found"}
@mcp.tool()
def bina_export_assets(asset_ids:list[str]):
 p,items=export_assets(asset_ids);return {"path":str(p),"assets":items}
@mcp.tool()
