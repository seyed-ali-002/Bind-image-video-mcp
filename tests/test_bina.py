...
def test_connection_auth():
 ...
 r=client.get('/connection',headers={'Authorization':f'Bearer {settings.auth_token}'})
 assert r.status_code==200 and r.json()['url'].endswith('/mcp')
def test_image_generation():
 r=generate_image('test',64,64); assert Path(r['path']).exists()
def test_video_job():
 job=generate_video('test',1); import time
 for _ in range(20):
